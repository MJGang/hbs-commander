...
describe('appendRight模式测试', () => {
  const targetDir = path.join(__dirname, 'target')
  const templateDir = path.join(__dirname, 'template')
  const testFiles = [
    {
      template: {
        filename: 'appendRight.hbs',
        content: `
{{!-- appendRight :row="2" :col="15" --}}
// appendRight内容
{{!-- /appendRight --}}
`,
      },
      target: {
        filename: 'appendRight.vue',
        content: `<template>
  <div>测试</div>
</template>`,
      },
  ...
  it('1. appendRight--追加内容', async ({ targetPath, templateFilename, targetFilename }) => {
    await hbscmd({
    ...
    expect(result).toContain('  <div>测试</div>// appendRight内容')
  })
})
